...
(module kingsmint GOVERNANCE
  ...
  (defun add-policy:string (collection:string policy:object{policy-data})
    ...
    (let 
      (
        ...
        (policy-stack:string (at "policy-stack" policy))
      )
  ...
  (defun mint-all-lords:string 
    ...
    (with-capability (OPS) 
      ...
      (enforce (= amount 50) "Must mint an exact number of lords")
      (map (mint-lord collection account) (enumerate 1 50))
    )
      ...
      (with-read minted-tokens (get-mint-token-id collection marmToken)
        ...
        (let*
          (
            ...
            (accountType:string (check-whitelist-for-type collection "king" account))
          )
          ...
          (with-read policies (concat [collection "-" accountType])
            ...
            (let
              (
                (policy:[module{marmalade-ng.token-policy-ng-v1}] (marmalade-ng.std-policies.to-policies policy-stack))
              )
              ;; Create the token (NG)
              (create-token token-id precision uri policy guard)
              ;; Write to NG ledger
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ;; Check-whitelist-for-type     
  ;;-------------------------------------------------
  ;; Checks a whitelister for their type.  This is Used
  ;; to check if a whitelister is a king.
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;   
  (defun check-whitelist-for-type:string (collection:string type:string account:string)
    @doc "Selects a token by account."
    (let 
      (
        (rowset:[object{whitelisted}] (select whitelist-table (where "account" (= account))))
      )
      ;;the row should be exctly 1 element long!
  ...
  ;; Is-whitelisted
  ;;-------------------------------------------------
  ;; Checks if an account has a certain NFT type in 
