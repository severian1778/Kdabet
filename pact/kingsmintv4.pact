...
(namespace "n_5382b05312493cdadba55b2928f839127f3f1a7e")

;; Define the critical keysets that are need to run smart contract operations
;(define-keyset "n_5382b05312493cdadba55b2928f839127f3f1a7e.kdabet_admin" (read-keyset "kdabet_admin"))
;(define-keyset "n_5382b05312493cdadba55b2928f839127f3f1a7e.kdabet_operations" (read-keyset "kdabet_operations"))

(module kingsmintv8 GOVERNANCE
  ...
  (defcap GOVERNANCE ()
    (enforce-guard (keyset-ref-guard "n_5382b05312493cdadba55b2928f839127f3f1a7e.kdabet_admin"))
  ...
  (defcap OPS()
    (enforce-guard (keyset-ref-guard "n_5382b05312493cdadba55b2928f839127f3f1a7e.kdabet_operations"))
  ...
  (defcap WLCREATOR:bool (collection:string)
    ...
    (util.guards.enforce-or creatorGuard  (keyset-ref-guard "n_5382b05312493cdadba55b2928f839127f3f1a7e.kdabet_operations") ))
  ...
  (defschema token-data
    precision:integer
    uri:string
    policy:[module{n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.token-policy-ng-v1}]
  )
    ...
    (with-capability (CREATECOL)
      ...
      (let*
        (
          ...
          (collection-id:string (n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.policy-collection.create-collection-id name operator-guard))
        )
            ...
            (
              (actual-cost:decimal 
                (if 
                  ...
                  (let 
                    (
                      (kdausdprice:decimal (at "kda-usd-price" (free.pay-oracle.get-kda-usd-price)))
                    ) 
        ...
        (let*
          ...
          (with-read policies (concat [collection "-" accountType])
            ...
            (let
              ...
              ;; Create the token (NG)
              (n_442d3e11cfe0d39859878e5b1520cd8b8c36e5db.ledger.create-token token-id precision uri policy guard)
              ;; Write to NG ledger
  ...
  (defun add-to-whitelist:string
    ...
    (with-read collections collection 
      ...
      (let* 
        ...
        (enforce (or mintstatus hasminted) (format "The wallet {} has already whitelisted" [account]))
